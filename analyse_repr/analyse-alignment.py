...
class Sample:
	...
	def __init__(self, p, p_act, p_rep, h, h_act, h_rep, lbl):
		self.p = p
		self.h = h
		self.p_act = p_act
		self.h_act = h_act
...
def analyse_sent_alignment(params):
	...
	def score_num_act(idx_p, idx_h, sample, print_out=False):
		...
		act_dims = get_shared(idx_p, idx_h, sample)
		if print_out:
...
def main():
	...
	Usage:
		...
		analyse-alignment.py cm <sent_idx> <label> <conf_type> [--pt=<print_top>] [--t=<threshold>] [--tb=<threshold_both>] [--tsd=<top_sd>]
