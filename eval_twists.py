...
def plot_results(output_file):
	...
	def create_simple_dict(labels):
		d = dict()
		for lbl_gold in labels:
			d[lbl_gold] = dict()
			for lbl_pred in labels:
				d[lbl_gold][lbl_pred] = 0
		return d

	with open(output_file) as f_in:
		data = [line.strip().split(' ') for line in f_in.readlines()]

	data = [(v[0].split('-'), int(v[1])) for v in data]
	data = [(lbl[0], lbl[1], lbl[2], amount) for lbl, amount in data]

	x_labels = mydataloader.index_to_tag

	# Plot original model
	d = create_simple_dict(x_labels)
	for lbl_gold, lbl_pred, _, amount in data:
		d[lbl_gold][lbl_pred] += amount
	plot_correct_incorrect_bar(x_labels, d, 'Normal model', block=False)

	# Plot twisted model
	d = create_simple_dict(x_labels)
	for lbl_gold, _, lbl_pred, amount in data:
		d[lbl_gold][lbl_pred] += amount
	plot_correct_incorrect_bar(x_labels, d, 'Changed representation model')
...
def main():
	...
	if args['plot'] != None:
		return plot_results(args['<output>'])
