'''To work with model prior to refactorisation'''
...
from libs import embeddingholder, config, data_tools
...
class Compatablility:
    '''
    Model are available using this compatability class.
    '''
    ...
    def load_model(self, path, embedding_holder=None):
        if embedding_holder == None:
            embedding_holder = embeddingholder.create_embeddingholder()

        classifier, model_name = load_model(path, embedding_holder=embedding_holder)

        return (model_name, classifier, embedding_holder)

    def get_datahandler_train(self):
        # use nltk tokenization
        return data_tools.Datahandler(config.PATH_TRAIN_DATA, data_format='snli_nltk')
